...
def get_state(pacman_x, pacman_y, ghost_x, ghost_y, score):
    ...
    return np.array([[pacman_x], [pacman_y], [ghost_x], [ghost_y], [score], [distance]])
...
def train_ai(W1, b1, W2, b2, episodes=1000):
    ...
    for episode in range(episodes):
        ...
        score = 0
        points = [(random.randint(20, 580), random.randint(20, 580)) for _ in range(10)]
        ...
        for i in range(len(states)):
            ...
            total_loss += -float(np.log(A2[actions[i]])) * disc_rewards[i]
